<?php
/* 
Plugin Name: Smart Product Gallery Slider
Plugin URI: https://shameemreza.com/woocommerce-smart-product-gallery-slider/
Description: Smart Product gallery slider will allows you to display a nice image slideshow in the product pages instead of the default lightbox.
Version: 1.0.0
Author:  Shameem Reza
Author URI: http://www.shameemreza.com
*/

  /* Plugin Constants */
define('SMART_WOO_SLIDER',plugin_dir_url(__FILE__));
define('SMART_WOO_SLIDER_CSS',SMART_WOO_SLIDER.'/css');
define('SMART_WOO_SLIDER_JS',SMART_WOO_SLIDER.'/js');
define('SMART_WOO_SLIDER_IMAGES',SMART_WOO_SLIDER.'/images');


/* register hook */

register_activation_hook(__FILE__,'tx_woo_slider_register_init');

function tx_woo_slider_register_init(){
...
'w_slider_resizeDuration'=>700,
       ...
       add_option('tx_woo_slider_settings',$settings_array);
...
function woo_slider_settings(){
	...
	$settings=get_option('tx_woo_slider_settings');
...
require_once('admin/smart_product_slider_menu.php');


/* required scripts and styles */


function tx_woo_slider_scripts(){   
    ...
    if(is_single() && is_product()){
        ...
        wp_enqueue_script('tx_woo_jsor_slider', SMART_WOO_SLIDER_JS.'/jssor.slider.mini-dots.js',array('jquery'),'1.0', false);

        else if($settings['w_slider_type']==='Thumbs')

        wp_enqueue_script('tx_woo_jsor_slider', SMART_WOO_SLIDER_JS.'/jssor.slider.mini-thumbs.js',array('jquery'),'1.0', false);

        else

          wp_enqueue_script('tx_woo_jsor_slider', SMART_WOO_SLIDER_JS.'/jssor.slider.mini-thumbs-vertical.js',array('jquery'),'1.0', false);

          	

         wp_localize_script( 'tx_woo_jsor_slider', 'txWooSliderParam', woo_slider_settings());
		 
		 
		if(isset($settings['w_slider_activate_lightbox']) && $settings['w_slider_activate_lightbox']=='yes' ){
			
			wp_enqueue_script('tx_woo_lightbox', SMART_WOO_SLIDER_JS.'/lightbox.js',array('jquery'),'1.0', true);
			wp_enqueue_script('lightbox_woo_action', SMART_WOO_SLIDER_JS.'/lightbox_woo_action.js',array('jquery','tx_woo_lightbox'),'1.0', true);
			
			 wp_localize_script( 'lightbox_woo_action', 'txWooLightBoxParam', woo_slider_lightbox_settings());
		}
...
add_action('wp_print_scripts', 'tx_woo_slider_scripts');



function tx_woo_slider_styles() {
  ...
  if(is_singular('product')){
      ...
      $settings=get_option('tx_woo_slider_settings');
     wp_enqueue_style('tx_woo_slider_style', SMART_WOO_SLIDER_CSS.'/front/smart_wooslider_styles.css',array() , '1.0.0', 'screen');
	 
	 
	 if(isset($settings['w_slider_activate_lightbox']) && $settings['w_slider_activate_lightbox']=='yes' ){
			
			wp_enqueue_style('tx_woo_slider_lightbox_style', SMART_WOO_SLIDER_CSS.'/front/lightbox.css',array(),'1.0.0', 'screen');
		}
...
add_action('wp_print_styles', 'tx_woo_slider_styles');



function tx_woo_slider_admin_styles() {

     wp_enqueue_style('tx_woo_slider_admin_style', SMART_WOO_SLIDER_CSS.'/admin/smart_wooslider_admin_styles.css',array() , '1.0.0', 'screen');

}

add_action('admin_print_styles', 'tx_woo_slider_admin_styles');
